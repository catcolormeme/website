...
export default function AkkaSwapModalFooter({
  ...
  const realizedLPFee = fee
    ? fee.reduce((accumulator, value) => {
  ...
  return (
    <>
      <SwapModalFooterContainer>
        ...
        <RowBetween marginY={3}>
          <RowFixed>
            ...
            <QuestionHelper
              text={t(
                `These are the estimated additional taxes introduced by the token project, not by ${process.env.NEXT_PUBLIC_NAME}.`,
              )}
              ml="4px"
              placement="top-start"
            />
          ...
          <Text fontSize="14px">
            {realizedLPFee
              ? `${realizedLPFee?.toFixed(6)} ${
                  trade?.route?.routes[chainId.toString()][0].routes[0].operationsSeperated[0].operations[0]
                    .offerToken[3]
                }`
              : '-'}
          </Text>
      ...
      <AutoRow>
        {isLoading ? (
          <CommitButton variant="primary" width="100%" disabled mt="12px">
