...
export const Kyc: React.FC = () => {
  ...
  if (status === 'connected' && paid.data) {
    if (paid.data === 'payed')
      action = (
        <Link href="https://whalechain.synaps.me" passHref legacyBehavior>
      ...
      action = (
        <Flex alignItems="center" flexDirection="column" gap="1em">
          ...
          <AddToWallet
            ...
            tokenSymbol="ICEKYC"
            tokenDecimals={0}
            tokenImage="https://catcolor.live/kyc.png"
            variant="success"
          >
    ...
    <Box background={isDark ? 'linear-gradient(135deg, #1d1c21 0%, #141317 100%)' : undefined}>
      ...
      <Page style={{ maxWidth: '800px' }}>
        <Flex flexDirection="column" gap="0.75em">
          <Text>
            {t(
              `The ${process.env.NEXT_PUBLIC_NAME} KYC-Soulbound Token allows you to publicly prove you have been KYCed with ${process.env.NEXT_PUBLIC_NAME}. For projects that means you can get access to extra functionality and greatly increase your communities and other projects trust. For Users this means you might receive AirDrops where the KYC token is a prerequisite so projects are sure they don't AirDrop to bots.`,
            )}
          </Text>
          <Text>
            {t(
              'To get your token, you will need to pay a fee of %fee% %symbol%. After you pay, you will be redirected to a KYC form, where you will need to provide your identity information. After you submit the form, your token will be sent to your wallet. The whole process just takes a few minutes.',
              { fee: chain.kyc?.fee, symbol: token?.symbol },
            )}
          </Text>
