...
const stepsConfigBuilder = ({ t, userInfos, eventInfos, userStatus, account, theme, cakeBalance }: EventStepsType) => {
  ...
  return [
    ...
    },
    {
      ...
      title: t(`Hold ${process.env.NEXT_PUBLIC_NAME_UPPER}`),
      bodyText: [
        t(
          `Each NFT costs ${process.env.NEXT_PUBLIC_NAME_UPPER} to mint. Remember you also need BNB to cover transaction fees too!`,
        ),
      ],
      buttons: (
        <>
          <Button
            as="a"
            href="https://whalechain.live/swap?outputCurrency=0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82&chainId=56"
            target="_blank"
            width="100%"
          >
            {t(`Buy ${process.env.NEXT_PUBLIC_NAME_UPPER}`)}
          </Button>
        </>
      ),
    },
