      ...
      {!checkedState && (
        <Message variant="warning" mb="16px">
          <MessageText maxWidth="320px">
            {t(
              `Adding more ${process.env.NEXT_PUBLIC_NAME_UPPER} will renew your lock, setting it to remaining duration. Due to shorter lock period, benefits decrease. To keep similar benefits, extend your lock.`,
            )}
          </MessageText>
        </Message>
      )}
...
const AddAmountModal: React.FC<React.PropsWithChildren<AddAmountModalProps>> = ({
  ...
  return (
    <RoiCalculatorModalProvider lockedAmount={lockedAmount}>
      <Modal
        title={t(`Add ${process.env.NEXT_PUBLIC_NAME_UPPER}`)}
        onDismiss={onDismiss}
