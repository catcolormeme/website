...
const getPathList = (t: ContextApi['t']): PathList => {
  return {
    paths: {
      ...
      '/farms': { title: t('Farms') },
      '/pools': { title: t('Pools') },
      '/info': { title: t('Overview'), description: `View statistics for ${process.env.NEXT_PUBLIC_NAME} exchanges.` },
      '/info/pools': { title: t('Pools'), description: `View statistics for ${process.env.NEXT_PUBLIC_NAME} exchanges.` },
      '/info/tokens': { title: t('Tokens'), description: `View statistics for ${process.env.NEXT_PUBLIC_NAME} exchanges.` },
      '/core': { basePath: true, title: t('Get Ready for Core 🚀') },
      '/bridge': {
        basePath: true,
        title: t('Bridge'),
        description:
          `Transfer tokens between multiple Chains including Core, Bitgert, Binance, XDC, Fuse and may more on ${process.env.NEXT_PUBLIC_NAME} DEX.`,
      },
    },
    defaultTitleSuffix: t(process.env.NEXT_PUBLIC_NAME),
  }
}
