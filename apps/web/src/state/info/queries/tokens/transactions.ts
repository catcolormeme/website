...
const TOKEN_TRANSACTIONS = () => {
  ...
  return gql`
    query tokenTransactions($address: String!) {
      ...
      burnsAs0: burns(first: 10, orderBy: timestamp, orderDirection: desc, where: { token1: $address} ) {
        id
        timestamp
        pair {
          token0 {
      ...
      burnsAs1: burns(first: 10, orderBy: timestamp, orderDirection: desc, where: { token0: $address} ) {
        id
        timestamp
        pair {
          token0 {
